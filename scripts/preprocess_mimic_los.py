...
def preprocess_mimic_los_for_llm(batch, tokenizer, **llm_prompt_format):
    outcomes = []
    all_texts = []
    feature_explanation_text = ""
    if llm_prompt_format["feature_explanations"]:
        for feature_name, feature_explanation in llm_prompt_format["feature_explanations"].items():
            feature_explanation_text += f"{feature_name.upper()}: {feature_explanation}\n"
    for batch_item, batch_item_time_delta, batch_item_outcome in zip(batch["features"], batch["time_deltas"], batch["outcomes"]):
        batch_item_text = f"{feature_explanation_text}"
        for i, time_point in enumerate(batch_item):
            clinical_note = ""
            structured_data_text = "STRUCTURED DATA:\n"
            for feature_type in time_point:
                if feature_type == "clinical_note":
                    if "text" in time_point[feature_type]:
                        clinical_note = '\nCLINICAL NOTE: """\n' + time_point[feature_type]["text"] + '\n"""'
                else:
                    for feature_name, feature_value in time_point[feature_type].items():
                        ...
                        if llm_prompt_format["feature_explanations"]:
                            feature_explanation = llm_prompt_format["feature_explanations"][feature_name]
                            structured_data_text += f"{feature_explanation}\n"
                        structured_data_text += f"{feature_name.upper()}: {feature_value}\n"
            time_delta_text = f"TIME DELTA: {int(batch_item_time_delta[i])}\n"
            batch_item_text += f"{clinical_note}\n\n{structured_data_text}\n{time_delta_text}"
            batch_item_text += "\nEND OF TIME POINT\n"
        batch_item_text += tokenizer.eos_token
        outcomes.append(batch_item_outcome["los"][0])
...
def preprocess_mimic_los(
    model: str, data_path: str, out_path: str, vocabs_path: str, device_id: int | None, jina_task: str, llm_prompt_format: dict
):
    ...
    if model == "weighted_lstm":
        ...
        tokenizer = AutoTokenizer.from_pretrained(vocabs_path)
        tokenizer.pad_token = tokenizer.eos_token
        for split in datasets.keys():
            datasets[split] = datasets[split].map(
                lambda x: preprocess_mimic_los_for_llm(x, tokenizer, **llm_prompt_format),
                batched=True,
                remove_columns=datasets[split].column_names,
            )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="The path to the tokenizer(s) vocabolary or tokenizer path. Required for weighted_lstm, tann, llm and temporal_recurrent_llm models.",
    )
    parser.add_argument(
        "--jina_task",
        type=str,
    ...
    if args.feature_explanations_path is not None:
        feature_explanations = json.load(open(args.feature_explanations_path))

    llm_prompt_format = {
        "truncate_decimals": args.truncate_decimals,
        "feature_explanations": feature_explanations,
    }
    preprocess_mimic_los(
        args.model, args.data_path, args.out_path, args.vocabs_path, args.device_id, args.jina_task, llm_prompt_format
    )
